...
_TOKEN_TTL = 60 * 60 * 7  # 7 hours (token valid for 8)
...
def _get_auth_token() -> str:
    ...
    resp = _get_session().post(
        f'{settings.LMS_BASE_URL}/api/auth/token/',
        json={
            'username': settings.LMS_SERVICE_USERNAME,
            'password': settings.LMS_SERVICE_PASSWORD,
        },
    ...
    if not token:
        raise ValueError('No access token in LMS auth response')
    cache.set(_TOKEN_KEY, token, timeout=_TOKEN_TTL)
    return token
...
def send_repayment(
    ...
    """
    Thread-safe repayment dispatch with:
      1. Per-phone Redis lock (prevents race conditions)
      2. Idempotency check (prevents duplicate sends)
      3. Balance cap (prevents negative balances)
      4. LMS POST
    """
    # 1. Acquire lock
    waited = 0
    while not _acquire_lock(phone_number):
        if waited >= max_wait_lock:
            ...
            return LMSRepaymentResult(
                ...
                duration_ms=0, error='Concurrent repayment in progress — skipped'
            )
        time.sleep(1)
        waited += 1
    ...
    try:
        ...
        try:
            ...
            log.info('LMS repayment | phone=%s amount=%s code=%s %dms',
            ...
            return LMSRepaymentResult(
                ...
                response_body=body, duration_ms=duration_ms,
...
def _cap_amount_to_balance(phone_number: str, requested: Decimal) -> Decimal:
    """Returns min(requested, total_active_loan_balance). Returns 0 if no active loans."""
    try:
        resp = _get_session().post(
        ...
        log.warning('Could not fetch balance for %s: %s — using requested', phone_number, exc)
        return requested  # conservative fallback; LMS will reject if truly over-balance
...
def _upsert_idempotency(key: str, organization_code: str, status: str, response_payload: dict):
